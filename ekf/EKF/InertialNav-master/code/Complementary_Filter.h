...
class Complementary_Filter
{
    ...
    float Beta_dot_i_0, Beta_dot_i_1, Beta_dot_i_2;
    // Depedent variables
    ...
    {   return ((un2+2*un1+un-yn1*2*(1+Z)*(1-Z)-yn*(1-Z)*(1-Z))/(1+Z)/(1+Z)); }

    inline float high_pass(float un, float un1, float un2, float yn, float yn1)
    {   return (2*Z*(un2*(1/2*Z+1)+un1*(2-Z)+un*(1-Z/2))-2*yn*(1+Z)*(1-Z)-yn*(1-Z)*(1-Z))/((1+Z)*(1+Z));   }
    
    public:
    // Class Constructor
    Complementary_Filter(float T = 0.015, float Tau = 3): 
    T(T), Tau(Tau), g(32.2), v(0.0), Theta(0.0), Phi(0.0), ny(0.0), p(0.0), r(0.0),
    sine_G(0.0), V_1(0.0), V_2(0.0), V_dot_1(0.0), V_dot_2(0.0), 
    h_1(0.0), h_2(0.0), h_dot_1(0.0), h_dot_2(0.0), 
    Beta_dot_i_0(0.0), Beta_dot_i_1(0.0), Beta_dot_i_2(0.0),
    Beta_if_0(0.0), Beta_if_1(0.0), Beta_if_2(0.0),
    ...
    void init(float Theta_l, float Phi_l, float ny_l, float r_l, float p_l, float v_l, float Alpha_v_l, float V_1_l, float h_1_l)
    {
        ...
        Alpha_i_1 = cos(Phi) != 0 ? Alpha_Inertial(sine_G, Theta, Beta_cf_1, Phi) : Alpha_i_1;
        Beta_dot_i_1 = V_1 > 0 ? Beta_dot_Inertial(g, V_1, Beta_cf_1, V_dot_1, ny, Theta, Phi, r, Alpha_i_1, p) : Beta_dot_i_1;
    ...
    void runFilter(float Theta_l, float Phi_l, float ny_l, float r_l, float p_l, float v_l, float Alpha_v_l, float V_2_l, float h_2_l)
    {
        ...
        // High-Pass Filter
        Beta_if_2 = high_pass(Beta_dot_i_0, Beta_dot_i_1, Beta_dot_i_2, Beta_if_0, Beta_if_1);
        // Vane Computations
        Beta_v_2 = V_2 > 0 ? atan(v/V_2) : Beta_v_2;
        // Low-Pass Filter
