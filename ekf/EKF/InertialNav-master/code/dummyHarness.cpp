...
int main(int argc, char *argv[])
{
    ...
    while (true) {
        ...
        for (unsigned i = 0; i < nreads; i++) {
            ...
            int numRead = 255;
            while(numRead >= 255 && keepReading){
                numRead = read(sockfd,buffer,255);
                std::string goodData = std::string(buffer).substr(0,numRead);
                inStr += goodData;
                // std::cout << goodData;
                // printf("\t%d\t%d\n",inStr.length(),numRead);
                goodData.clear();
            }
            ...
            while(keepParsing)
            {
                ...
                if(inStr[0] != '$')
                {
                    //read end of previous string until $ is found
                    inStr.erase(0,inStr.find("$"));
                    //std::cout << inStr << "\n";
                }
                if(inStr.substr(0,4) == "$GPS")
                {
                    // printf("GPS Received\n");
                    inStr.erase(0,4);
                    int secEnd = inStr.find('$');
                    std::string gpsString = inStr.substr(0,secEnd);
                    inStr.erase(0,secEnd);


                    nextComma = gpsString.find(',');
                    now = stof(gpsString.substr(0,nextComma));
                    float gpsDt = (now - GPStimestamp);
                    _ekf->updateDtGpsFilt(gpsDt);
                    GPStimestamp = now;
                    gpsString.erase(0,nextComma+1);

                    nextComma = 1;
                    _ekf->GPSstatus = stoi(gpsString.substr(0,nextComma));
                    gpsString.erase(0,nextComma+1);

                    nextComma = gpsString.find(',');
                    GPSmsec = stof(gpsString.substr(0,nextComma));
                    gpsString.erase(0,nextComma+1);

                    nextComma = gpsString.find(',');
                    _ekf->gpsLat = stof(gpsString.substr(0,nextComma));
                    gpsString.erase(0,nextComma+1);

                    nextComma = gpsString.find(',');
                    _ekf->gpsLon = stof(gpsString.substr(0,nextComma));
                    gpsString.erase(0,nextComma+1);

                    nextComma = gpsString.find(',');
                    _ekf->gpsHgt = stof(gpsString.substr(0,nextComma));
                    gpsString.erase(0,nextComma+1);

                    nextComma = gpsString.find(',');
                    gpsGndSpd = stof(gpsString.substr(0,nextComma));
                    gpsString.erase(0,nextComma+1);

                    nextComma = gpsString.find(',');
                    gpsCourse = deg2rad*stof(gpsString.substr(0,nextComma));
                    gpsString.erase(0,nextComma+1);

                    nextComma = gpsString.find(',');
                    gpsVelD = stof(gpsString.substr(0,nextComma));
                    gpsString.erase(0,nextComma+1);
                ...
                else if(inStr.substr(0,4) == "$IMU")
                {
                    // printf("IMU Received\n");//////////////////////////////

                    inStr.erase(0,4);
                    int secEnd = inStr.find("$");
                    std::string imuString = inStr.substr(0,secEnd);
                    inStr.erase(0,secEnd);
                    nextComma = imuString.find(',');
                    IMUtimestamp = stof(imuString.substr(0,nextComma));
                    imuString.erase(0,nextComma+1);
                    nextComma = imuString.find(',');
                    uint32_t tmpTime = (int)(stof(imuString.substr(0,nextComma)));
                    _ekf->dtIMU     = 0.001f*(tmpTime - IMUmsec);
                    IMUmsec = tmpTime;
                    imuString.erase(0,nextComma+1);

                    nextComma = imuString.find(',');
                    _ekf->angRate.x = stof(imuString.substr(0,nextComma));
                    imuString.erase(0,nextComma+1);

                    nextComma = imuString.find(',');
                    _ekf->angRate.y = stof(imuString.substr(0,nextComma));
                    imuString.erase(0,nextComma+1);

                    nextComma = imuString.find(',');
                    _ekf->angRate.z = stof(imuString.substr(0,nextComma));
                    imuString.erase(0,nextComma+1);

                    nextComma = imuString.find(',');
                    _ekf->accel.x = stof(imuString.substr(0,nextComma));
                    imuString.erase(0,nextComma+1);

                    nextComma = imuString.find(',');
                    _ekf->accel.y = stof(imuString.substr(0,nextComma));
                    imuString.erase(0,nextComma+1);

                    nextComma = imuString.find(',');
                    _ekf->accel.z = stof(imuString.substr(0,nextComma));
                    imuString.erase(0,nextComma+1);
