    ...
    switch(state){
      ...
      case 1:
        ...
        while(numRead < 7){
          ...
          if(latestRead > 0){
            int numWrittenRawSerial = fwrite(&rxBuf[numRead], sizeof(char), latestRead, rawSerialFile);
            numRead += latestRead;
          }
        }
...
//        break;  //uncomment this and change "back" 3 lines above to "front" to use all values from arduino even if old
      ...
      int numWritten = sprintf(writeBuffer,"%lu %9.4f %7.2f %7.2f %9.4f %7.2f %7.2f %12.8f %12.8f %7.1f %6.3f %6.3f %6.3f %02u:%02u:%02u\n",
                                             cT, Alpha, pfwd, p45, latestArduinoData.alpha, latestArduinoData.pfwd, latestArduinoData.p45,
                                             ((float)gpsData.lat)*1E-7, ((float)gpsData.lon)*1E-7, ((float)gpsData.height)*1E-3, ((float)gpsData.velN*1E-3), ((float)gpsData.velE)*1E-3, ((float)gpsData.velD)*1E-3, gpsData.hour,gpsData.minute, gpsData.second);
      try{
        int numWrittenDatalog = fwrite(writeBuffer, sizeof(char), numWritten, datalogFile);
...
//      printf("\n%6lu %9.4f %7.2f %7.2f    %9.4f %7.2f %7.2f",cT, Alpha, pfwd, p45, latestArduinoData.alpha, latestArduinoData.pfwd, latestArduinoData.p45);
      printf("\n%lu %9.4f %7.2f %7.2f %9.4f %7.2f %7.2f %12.8f %12.8f %7.1f %6.3f %6.3f %6.3f %02u:%02u:%02u",
                                             cT, Alpha, pfwd, p45, latestArduinoData.alpha, latestArduinoData.pfwd, latestArduinoData.p45,
                                             ((float)gpsData.lat)*1E-7, ((float)gpsData.lon)*1E-7, ((float)gpsData.height)*1E-3, ((float)gpsData.velN*1E-3), ((float)gpsData.velE)*1E-3, ((float)gpsData.velD)*1E-3, gpsData.hour,gpsData.minute, gpsData.second);
    }
  }
